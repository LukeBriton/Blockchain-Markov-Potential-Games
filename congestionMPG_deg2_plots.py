...
def full_experiment(runs,iters,eta,T,samples,flagQ):
    ...
    for k in range(runs):
        if flagQ == 0:
            policy_hist = policy_gradient([0.0001, 0.9999],iters,0.99,eta,T,samples)
        if flagQ == 1:
            policy_hist = policy_gradientQ([0.0001, 0.9999], iters, 0.99, eta, T, samples)
...
plt.show()
fig2.savefig('Fig_individual_runs_n{}_deg2.png'.format(N), bbox_inches='tight')
...
# plt.gca().set(xlabel='Iterations',ylabel='L1-accuracy', title='Policy Gradient: agents = {}, runs = {}, $\eta$ = {}'.format(N, runs,eta))
plt.show()
fig1.savefig('Fig_avg_runs_n{}_deg2.png'.format(N), bbox_inches='tight')
# plt.close()
...
plt.legend()
fig3.savefig('Fig_facilities_n{}_deg2.png'.format(N), bbox_inches='tight')
# plt.close()
plt.show()
...
plt.xticks(index + bar_width / 2, ('A', 'B', 'C', 'D'))
plt.legend()
fig4.savefig('Fig_facilitiesQ_n{}_deg2.png'.format(N), bbox_inches='tight')
# plt.close()
plt.show()
